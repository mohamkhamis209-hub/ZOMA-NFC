...
insert into public.designs(design_code,name,price,nfc_available,qr_available,is_available) values
('DES-001','Black Luxury',350,true,false,true),('DES-002','Gold Premium',450,true,false,true),('DES-003','Business Blue',400,true,false,true)
...
create or replace function public.zoma_get_public_card(p_card_id text) returns table(card_uuid uuid,card_id text,status public.card_status,full_name text,photo_url text,facebook_url text,tiktok_url text,whatsapp_1 text,whatsapp_2 text,show_whatsapp_2 boolean) language sql stable security definer set search_path=public as $$
 select c.id,c.card_id,c.status,p.full_name,p.photo_url,p.facebook_url,p.tiktok_url,p.whatsapp_1,p.whatsapp_2,p.show_whatsapp_2 from public.cards c left join public.profiles p on p.card_id=c.id where c.card_id=p_card_id and c.status<>'suspended' limit 1;
$$;
...
drop policy if exists profiles_customer on public.profiles;drop policy if exists profiles_admin on public.profiles;create policy profiles_customer on public.profiles for select to authenticated using(exists(select 1 from public.cards c where c.id=profiles.card_id and c.customer_id=auth.uid()));create policy profiles_customer_write on public.profiles for insert to authenticated with check(exists(select 1 from public.cards c where c.id=profiles.card_id and c.customer_id=auth.uid()));create policy profiles_customer_update on public.profiles for update to authenticated using(exists(select 1 from public.cards c where c.id=profiles.card_id and c.customer_id=auth.uid())) with check(exists(select 1 from public.cards c where c.id=profiles.card_id and c.customer_id=auth.uid()));create policy profiles_admin on public.profiles for all to authenticated using(public.is_admin()) with check(public.is_admin());
